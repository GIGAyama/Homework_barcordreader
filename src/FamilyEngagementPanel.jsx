...
export default function FamilyEngagementPanel({ db, showToast }) {
  ...
  if (db.students.length === 0) {
    return <div className="bg-white border border-dashed border-slate-300 rounded-3xl p-12 text-center"><Users size={44} className="mx-auto text-slate-600 mb-3" /><h3 className="font-bold text-slate-700">家庭連携を始める準備</h3><p className="text-sm text-slate-600 mt-2">先に名簿管理で児童を登録してください。</p></div>;
  }

  return (
    <div className="space-y-4 animate-fade-in-up">
      <div className="bg-gradient-to-br from-blue-800 via-blue-800 to-cyan-700 rounded-3xl p-6 text-white shadow-lg shadow-blue-100 relative overflow-hidden">
        <MessageSquare size={170} className="absolute -right-8 -bottom-12 opacity-10" />
        <div className="relative z-10 max-w-3xl">
          <span className="inline-flex items-center gap-1.5 text-xs font-bold bg-black/25 px-3 py-1 rounded-full mb-3"><Sparkles size={14} /> 家庭との合意を次の支援へ</span>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[310px_minmax(0,1fr)] gap-4 items-start">
        <aside className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden xl:sticky xl:top-2">
          <div className="p-4 bg-slate-50 border-b border-slate-100">
            <h4 className="font-bold text-slate-700 flex items-center gap-2"><CalendarCheck size={18} className="text-blue-700" /> 今日の家庭フォロー</h4>
            <p className="text-xs text-slate-600 mt-1">確認予定日を迎えた記録</p>
          </div>
        ...
        <main className="space-y-4 min-w-0">
          <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div><span className="text-xs font-bold text-blue-700">SELECTED STUDENT</span><h4 className="text-xl font-bold text-slate-800 mt-1">{selectedStudent?.name} さん</h4></div>
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm"><span className="text-xs font-bold text-slate-600">直近90日の連携</span><p className="text-3xl font-bold text-slate-800 mt-2">{recentContacts.length}<span className="text-xs text-slate-600 ml-1">件</span></p></div>
            <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm"><span className="text-xs font-bold text-slate-600">連絡手段</span><p className="text-3xl font-bold text-slate-800 mt-2">{channelCount}<span className="text-xs text-slate-600 ml-1">種類</span></p></div>
            <div className="bg-white rounded-2xl border border-slate-100 p-4 shadow-sm"><span className="text-xs font-bold text-slate-600">要フォロー</span><p className={`text-3xl font-bold mt-2 ${pendingContacts.length ? 'text-amber-700' : 'text-slate-800'}`}>{pendingContacts.length}<span className="text-xs text-slate-600 ml-1">件</span></p></div>
          </div>

          <form onSubmit={saveContact} className="bg-white rounded-2xl border border-blue-100 shadow-sm p-5 space-y-5">
            <div><h5 className="font-bold text-slate-800 flex items-center gap-2"><Phone size={18} className="text-blue-700" /> 家庭とのやり取りを記録</h5><p className="text-xs text-slate-600 mt-1">評価ではなく、共有した事実と合意事項を具体的に残します。</p></div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <label className="space-y-1.5"><span className="text-xs font-bold text-slate-600">連絡日</span><input type="date" value={form.date} onChange={event => updateForm({ date: event.target.value })} className={fieldClass} /></label>
              <label className="space-y-1.5"><span className="text-xs font-bold text-slate-600">連絡手段</span><select value={form.channel} onChange={event => updateForm({ channel: event.target.value })} className={fieldClass}>{CHANNELS.map(channel => <option key={channel}>{channel}</option>)}</select></label>
              <label className="space-y-1.5"><span className="text-xs font-bold text-slate-600">話題</span><select value={form.topic} onChange={event => updateForm({ topic: event.target.value })} className={fieldClass}>{TOPICS.map(topic => <option key={topic}>{topic}</option>)}</select></label>
            </div>
            <label className="space-y-1.5 block"><span className="text-xs font-bold text-slate-600">学校から共有した事実</span><textarea rows="3" value={form.sharedFacts} onChange={event => updateForm({ sharedFacts: event.target.value })} placeholder="例：直近2週間で、算数の用具を3回忘れた記録があります" className={fieldClass} /></label>
            ...
            <label className="space-y-1.5 block"><span className="text-xs font-bold text-slate-600">学校・家庭で合意した対応</span><textarea rows="3" value={form.agreement} onChange={event => updateForm({ agreement: event.target.value })} placeholder="例：同じ持ち物チェック表を使い、2週間後に変化を確認する" className={fieldClass} /></label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <label className="space-y-1.5"><span className="text-xs font-bold text-slate-600">次回確認日（空欄なら完了）</span><input type="date" value={form.followUpDate} onChange={event => updateForm({ followUpDate: event.target.value })} className={fieldClass} /></label>
              <label className="space-y-1.5"><span className="text-xs font-bold text-slate-600">対応者</span><input value={form.staffName} onChange={event => updateForm({ staffName: event.target.value })} placeholder="例：担任、養護教諭" className={fieldClass} /></label>
            </div>
            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-600 text-white font-bold py-3.5 rounded-xl shadow-md transition-all active:scale-[0.99] flex items-center justify-center gap-2"><Plus size={18} /> 家庭連携を記録</button>
          </form>
          ...
          <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
            ...
            {selectedContacts.length === 0 ? <div className="p-10 text-center text-sm font-bold text-slate-600">家庭連携の記録はまだありません</div> : (
                ...
                return (
                  <article key={contact.id} className="p-5">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mt-4 text-xs">
                      ...
                      <div className="bg-blue-50 rounded-xl p-3"><span className="font-bold text-blue-700">合意した対応</span><p className="font-bold text-blue-900 mt-1 leading-relaxed">{contact.agreement}</p></div>
                    </div>
                    <div className="flex flex-wrap gap-3 mt-3 text-xs text-slate-600 font-bold"><span>{formatDate(contact.date)}・{contact.staffName || '対応者未記入'}</span>{contact.followUpDate && <span className="flex items-center gap-1"><Clock3 size={13} /> {formatDate(contact.followUpDate)}確認</span>}</div>
                    ...
                    {contact.status !== '完了' && (
                      <div className="mt-4 bg-slate-50 border border-slate-100 rounded-xl p-4">
                        <div className="font-bold text-sm text-slate-700 flex items-center gap-2 mb-3"><Handshake size={17} className="text-blue-700" /> 次回確認を記録</div>
