...
const Trend = ({ value, unit = '' }) => {
  if (value === null || value === undefined) return <span className="text-[10px] font-bold text-slate-600">前期間の比較なし</span>;
  return <span className="text-[10px] font-bold text-slate-600">前の14日間から {value > 0 ? '+' : ''}{value}{unit}</span>;
};
...
export default function ClassInsightsPanel({
  ...
  return (
    <div className="space-y-4 animate-fade-in-up">
      <div className="bg-gradient-to-br from-teal-800 via-cyan-800 to-indigo-800 rounded-3xl p-6 text-white shadow-lg shadow-cyan-100 relative overflow-hidden">
        <Activity size={170} className="absolute -right-8 -bottom-12 opacity-10" />
        <div className="relative z-10 max-w-3xl">
          <span className="inline-flex items-center gap-1.5 text-xs font-bold bg-black/25 px-3 py-1 rounded-full mb-3"><Sparkles size={14} /> 記録を学級改善へ</span>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-3">
        <MetricCard label="課題提出率" value={metrics.submission.rate === null ? '—' : `${metrics.submission.rate}%`} note={`${metrics.submission.submitted}/${metrics.submission.required}回`} Icon={ClipboardCheck} iconClass="text-emerald-700" trend={metrics.submission.delta} unit="pt" />
        <MetricCard label="困り感のある選択" value={metrics.feelings.challengingRate === null ? '—' : `${metrics.feelings.challengingRate}%`} note={`${metrics.feelings.challenging}/${metrics.feelings.total}件`} Icon={HeartPulse} iconClass="text-orange-700" trend={metrics.feelings.delta} unit="pt" />
        <MetricCard label="忘れ物" value={`${metrics.forgotten.total}件`} note={metrics.forgotten.topItems[0] ? `最多：${metrics.forgotten.topItems[0].label}` : '期間内の記録'} Icon={Backpack} iconClass="text-red-700" trend={metrics.forgotten.delta} unit="件" />
        <MetricCard label="支援の確認期限" value={`${metrics.support.dueFollowUps}件`} note="予定日を迎えた実施中支援" Icon={CalendarCheck} iconClass="text-indigo-700" />
      </div>

      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50 flex items-center justify-between gap-3">
          <div>
            <h4 className="font-bold text-slate-800 flex items-center gap-2"><Lightbulb size={19} className="text-amber-700" /> 今週の改善候補</h4>
            <p className="text-xs text-slate-600 mt-1">基準に該当した集計事実と、学級全体で試せる小さな一手</p>
          </div>
          <span className="text-xs font-bold bg-white border border-slate-200 px-3 py-1.5 rounded-full text-slate-600">{analysis.insights.length}件</span>
        </div>
            ...
            return (
              <article key={insight.id} className={`rounded-2xl border p-5 ${insight.priority >= 3 ? 'border-red-200 bg-red-50/60' : insight.priority === 2 ? 'border-amber-200 bg-amber-50/60' : 'border-emerald-200 bg-emerald-50/60'}`}>
                <div className="flex items-start justify-between gap-3">
                  <div><span className="text-[10px] font-bold bg-white border border-slate-200 text-slate-600 px-2 py-1 rounded-full">{insight.area}</span><h5 className="font-bold text-slate-800 mt-3">{insight.title}</h5></div>
                  {insight.priority >= 3 && <span className="text-[10px] font-bold bg-red-100 text-red-700 px-2 py-1 rounded-full">優先確認</span>}
                </div>
                <div className="mt-4 space-y-3 text-xs leading-relaxed">
                  <div><span className="font-bold text-slate-600">根拠</span><p className="text-slate-700 mt-1">{insight.evidence}</p></div>
                  <div><span className="font-bold text-teal-700">試すこと</span><p className="font-bold text-slate-800 mt-1">{insight.action}</p></div>
                  <div><span className="font-bold text-indigo-700">確認指標</span><p className="text-slate-700 mt-1">{insight.measure}</p></div>
                </div>
      ...
      <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
        <div className="p-5 border-b border-slate-100 bg-slate-50">
          <h4 className="font-bold text-slate-800 flex items-center gap-2"><RefreshCw size={18} className="text-teal-700" /> 学級改善サイクル</h4>
          <p className="text-xs text-slate-600 mt-1">実施した工夫を14日後に振り返り、次の学級運用へつなげます。</p>
        </div>
        {activeActions.length === 0 ? (
          <div className="p-10 text-center">
            <Users size={38} className="mx-auto text-slate-600 mb-3" />
            <p className="font-bold text-slate-600">実施中の学級改善プランはありません</p>
            <p className="text-xs text-slate-600 mt-1">改善候補から、今週試すことを一つ選んでください。</p>
          </div>
        ) : (
              ...
              return (
                <article key={action.id} className="p-5">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <button type="button" onClick={() => deleteAction(action)} aria-label="改善プランを削除" className="p-2 text-slate-600 hover:text-red-700 rounded-lg"><Trash2 size={16} /></button>
                  </div>
                  <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mt-4 text-xs">
                    <div className="bg-slate-50 rounded-xl p-3"><span className="font-bold text-slate-600">根拠</span><p className="text-slate-700 mt-1 leading-relaxed">{action.evidence}</p></div>
                    <div className="bg-teal-50 rounded-xl p-3"><span className="font-bold text-teal-700">実施すること</span><p className="font-bold text-teal-900 mt-1 leading-relaxed">{action.action}</p></div>
                    <div className="bg-indigo-50 rounded-xl p-3"><span className="font-bold text-indigo-700">確認指標</span><p className="text-indigo-900 mt-1 leading-relaxed">{action.measure}</p></div>
                  </div>
                  <div className={`mt-3 text-xs font-bold flex items-center gap-2 ${overdue ? 'text-red-700' : 'text-slate-600'}`}><CalendarCheck size={14} /> {action.reviewDate} に振り返り{overdue ? '（確認時期です）' : ''}</div>
                  <div className="mt-4 bg-slate-50 border border-slate-100 rounded-xl p-4">
                    <div className="flex items-center gap-2 text-sm font-bold text-slate-700 mb-3"><AlertCircle size={17} className="text-teal-700" /> 実施後の変化</div>
                    ...
                    <button type="button" onClick={() => saveOutcome(action)} className="w-full lg:w-auto bg-teal-600 hover:bg-teal-600 text-white text-sm font-bold px-5 py-2.5 rounded-xl mt-3">振り返りを保存</button>
                  </div>
                </article>
              );
            })}
      ...
      {completedActions.length > 0 && (
        <section className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
          ...
          <div className="divide-y divide-slate-100">{completedActions.slice(0, 8).map(action => <div key={action.id} className="p-4 flex flex-col md:flex-row md:items-center justify-between gap-2 text-sm"><div><span className="font-bold text-slate-700">{action.title}</span><p className="text-xs text-slate-600 mt-1">{action.result}</p></div><span className="text-xs font-bold bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-full self-start">{action.outcomeRating}</span></div>)}</div>
        </section>
      )}

      <div className="text-xs text-slate-600 bg-cyan-50 border border-cyan-100 rounded-xl p-4 leading-relaxed">
